...
const Footer = () => {
  ...
  return (
    <footer className="bg-[#16112c] text-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="flex flex-col lg:flex-row items-start justify-between gap-8 sm:gap-10 py-8 sm:py-10 border-b border-white/10">
          <div className="max-w-md">
            ...
            >
              ...
              <span className="text-[#1c75bc] text-4xl sm:text-5xl leading-0">
                .
              </span>
            </Link>
            <p className="max-w-[410px] mt-4 sm:mt-6 text-xs sm:text-sm leading-relaxed text-gray-300">
            ...
            <div className="flex items-center gap-3 mt-4 sm:mt-5">
              {socialIcons.map((item, i) => (
                <Link
                  href={item.link}
                  key={i}
                  className="flex items-center justify-center w-9 h-9 sm:w-10 sm:h-10 bg-white/10 hover:scale-105 hover:bg-[#1c75bc] border border-white/20 hover:border-[#1c75bc] transition rounded-full"
                >
                  <item.icon />
                </Link>
              ))}
            ...
            <div className="mt-6 pt-6 border-t border-white/10">
              ...
              <div className="flex items-center gap-4 flex-wrap">
                <div className="px-3 py-2 bg-white/10 rounded text-xs font-medium text-white">
                  💳 Bank Transfer
                </div>
                <div className="px-3 py-2 bg-white/10 rounded text-xs font-medium text-white">
                  🔒 SSL Secure
          ...
          <div className="flex flex-wrap justify-between w-full lg:w-auto lg:flex-1 lg:max-w-2xl gap-8 sm:gap-10 text-sm">
            {linkSections.map((section, index) => (
              <div key={index} className="min-w-[140px]">
                <h3 className="font-semibold text-white mb-3 sm:mb-5">
                  {section.title}
                </h3>
                <ul className="space-y-2 sm:space-y-2.5">
                  {section.links.map((link, i) => (
                    ...
                    >
                      ...
                      <Link
                        href={link.path}
                        className="hover:text-[#1c75bc] hover:underline transition text-xs sm:text-sm"
                      >
                        {link.text}
                      </Link>
                    </li>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between py-4 sm:py-5 gap-3 text-xs sm:text-sm text-gray-400">
          <p>Copyright 2025 © PakCards. All Rights Reserved.</p>
          <div className="flex items-center gap-4 sm:gap-6">
            <Link href="/privacy" className="hover:text-[#1c75bc] transition">
              Privacy Policy
            </Link>
            <Link href="/terms" className="hover:text-[#1c75bc] transition">
              Terms of Service
            </Link>
          </div>
        </div>
